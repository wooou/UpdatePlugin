 ...
 *
 * <p>触发逻辑：当检查到有新版本更新时且配置的更新策略{@link UpdateStrategy#isShowUpdateDialog(UpdateInterface)}设定为true时。此通知创建器将被触发
 *
 * <p>定制说明：<br>
 *     1. 当需要进行后续更新操作时(请求进行apk下载任务)：调用{@link #sendDownloadRequest(UpdateInterface)}<br>
 *     2. 当需要取消此次更新操作时：调用{@link #sendUserCancel()}<br>
 *     3. 当需要忽略此版本更新时：调用{@link #sendUserIgnore(UpdateInterface)}<br>
 *
 * @author haoge
 */
public abstract class DialogCreator implements Recyclable {
     ...
     */
    public abstract Dialog create(UpdateInterface update, Activity context);

    /**
     * 当需要进行后续更新操作时：需要更新，启动下载任务时，调用此方法进行流程连接
     * @param update 更新数据实体类
     */
    protected void sendDownloadRequest(UpdateInterface update) {
     ...
     * @param update 更新数据实体类
     */
    protected void sendUserIgnore(UpdateInterface update) {
        if (this.checkCB != null) {
